      ...
      //  System.exit(0);
        ...
        try {
            for (PageDto page : pagesList) {
                ...
                if (insertQuery.length() >= 1000000) {
                    dbConnection.execSql("insert into pages(`code`, content, `path`, site_entity_id) values" + insertQuery);
                    insertQuery = new StringBuilder();
                }
            }
        ...
        for (Map.Entry<String, Integer> item : lemmaFrequency.entrySet()) {
            insertQuery.append(insertQuery.isEmpty() ? "" : ",")
                    ...
                    .append("')");
            if (insertQuery.length() >= 1000000) {
                dbConnection.execSql("insert into lemmas(lemma, frequency, site_entity_id) values" + insertQuery);
                insertQuery = new StringBuilder();
            }
        }
        ...
        for (IndexPage item : indexPageList) {
            insertQuery.append(insertQuery.isEmpty() ? "" : ",")
                    ...
                    .append("')");
            if (insertQuery.length() >= 1000000) {
                dbConnection.execSql("insert into indices(lemma_id, page_id, rank) values" + insertQuery);
                insertQuery = new StringBuilder();
            }
        }
