...
**Search:**   Раздел предназначен для поиска фразы на проиндексированных сайтах, или, отдельно указанном через выпадающий список, сайте. Для запуска поиска небходимо нажать кнопку "SEARCH". Также следует учитывать, что для сайта с большим количеством страниц вывод информации может зянять несколько минут (от 5 минут). Для сайтов с небольшим количеством страниц поиск может занимать несколько секунд. Информация по каждой найденной стрнице содержит заголовок страницы и сниппет, который будет содержать слова из опискового запроса. Наиболее релевантные страницы будут выведены в списке первыми сверху.
